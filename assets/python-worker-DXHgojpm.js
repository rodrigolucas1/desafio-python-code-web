import{loadPyodide as p}from"https://cdn.jsdelivr.net/pyodide/v314.0.4/full/pyodide.mjs";const f=String.raw`
...
def desafio_run_batch(code, lista_entradas):
    ...
    return resultados
`;let o=null;async function y(a){try{o=await p({indexURL:a||"https://cdn.jsdelivr.net/pyodide/v314.0.4/full/"}),o.runPython(f),self.postMessage({type:"loaded"})}catch(t){self.postMessage({type:"error",message:String(t&&t.message?t.message:t)})}}self.onmessage=async a=>{var n,i;const{type:t}=a.data||{};if(t==="init"){await y(a.data.indexURL);return}if(t==="run"){o||await new Promise(e=>setTimeout(e,50));try{const{code:e,inputs:u}=a.data,s=o.globals.get("desafio_run_batch"),d=[];for(const m of u||[]){const l=[];for(const _ of m||[])l.push(String(_));d.push(o.toPy(l))}const r=s(e,o.toPy(d)),c=r.toJs({create_proxies:!1});(n=r.destroy)==null||n.call(r),(i=s.destroy)==null||i.call(s),self.postMessage({type:"result",payload:{dados:c}})}catch(e){self.postMessage({type:"error",message:String(e&&e.message?e.message:e)})}}};
